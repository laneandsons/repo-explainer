...
`foundation` owns this file; no other build ticket may edit it.
"""
...
# Three limits with two different enforcers, because they answer different questions.
#
# MAX_SNIPPET_LINES is a grounding rule: a range longer than this is not trusted, so
# `ground` drops the snippet and counts the drop.
#
# MAX_SNIPPETS_PER_PAGE and SNIPPETS_ALLOWED_IN are page rules, not grounding rules —
# a fifth snippet, or one in the wrong section, is perfectly well grounded. `ground`
# deliberately leaves them alone: counting a budget drop as a grounding drop would mix
# "the model invented something" into the one number ADR 0001 exists to track. They
# belong to `render` (which lays the page out) and to `evals.gate`, which can check
# both off PageFacts alone. Flagged for a person in the foundation pull request.
