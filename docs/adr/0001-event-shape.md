...
Path and snippet drops are checked mechanically: is this path in the downloaded file list, does this
text appear character-for-character in that file. Claim drops are not. The model returns each claim
with the file it came from, and a claim citing no file is dropped and counted.
